...
<head>
    ...
    <style>
        * {
        ...
        .items .item td.to_left.max_length {
            max-width: 400px;
        }
        .table_sum_total {
            margin: 20px 0 40px;
...
<div id="body">
    ...
    <table class="table_main">
        ...
        <td class="half">
            ...
            <span class="line ico_dic">
                <span class="ico" style="margin-right: 15px;">@lang('invoice.identification_number'): {{$invoice->billed_from_client['identification_number']}}</span>
                <span class="dic">@if(!empty($invoice->billed_from_client['vat_identification_number']))@lang('invoice.vat_identification_number'): {{$invoice->billed_from_client['vat_identification_number']}}@endif</span>
            </span>
        ...
        <tr>
            <td>
                <table class="dates">
                    ...
                    @if($invoice->template_show_send_date)
                        <tr>
                            <td>@lang('invoice.delivery_date'):</td>
                            <td>{{date($dateFormat, strtotime($invoice->send_date))}}</td>
                        </tr>
                    @endif
                    </tbody>
                </table>
            ...
            <td>
                <span class="line">
                    <span class="ico"
                          style="margin-right: 20px;">@if(!empty($invoice->billed_to_client['identification_number']))@lang('invoice.identification_number'): {{$invoice->billed_to_client['identification_number']}}@endif</span>
                <span class="dic"
                      style="margin-right: 20px;">@if(!empty($invoice->billed_to_client['vat_identification_number']))@lang('invoice.vat_identification_number'): {{$invoice->billed_to_client['vat_identification_number']}}@endif</span>
               <span class="ic_dph">
                   @if(!empty($invoice->billed_to_client['vat_identification_number_sk']))
                       @lang('invoice.vat_identification_number_sk'): {{$invoice->billed_to_client['vat_identification_number_sk']}}
                   @endif
               </span>
                </span>
                            ...
                            <table>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <span class="line strong">@lang('invoice.sum'): <strong>{{number_format(round($showTax ? $invoice->totalPrice_with_tax : $invoice->totalPrice,2), 2, $decimalFormat, $thousandsFormat)}} {{$invoice->getCurrencySymbol()['symbol']}}</strong></span>
                                        <span class="line">@lang('invoice.variable_symbol'): <strong>{{$invoice->variable_symbol}}</strong></span>
                                        <span class="line">@if(!empty($invoice->bank_transfer['iban']))IBAN: <strong>{{$invoice->bank_transfer['iban'] ?? ''}}@endif</strong></span>
                                        <span class="line">@if(!empty($invoice->bank_transfer['swift']))SWIFT: {{$invoice->bank_transfer['swift'] ?? ''}}@endif</span>
                                        @if(!empty($nationalBankNumber))
                                            <span class="line">@if(!empty($nationalBankNumber))@lang('invoice.nationalBankNumber'): <strong>{{$nationalBankNumber}}</strong> @endif</span>
                                        @endif
                                    </td>
    ...
    <table class="items">
        ...
        <tr class="head">
            ...
            @if ($showTax)
                <td>@if($invoice->template_show_quantity)@lang('invoice.taxPercentage')@endif</td>
                <td>@if($invoice->template_show_quantity)@lang('invoice.tax')@endif</td>
            @endif
            <td>@lang('invoice.sum')
        ...
        @if(count($invoice->items))
            @foreach($invoice->items as $index => $item)
                ...
                <tr class="item">
                    ...
                    <td class="to_left max_length">{{$item['name']}}
                    </td>
    ...
    <table class="table_sum_total">
        ...
        @if($showTax)
            <tr>
                <td class="half"></td>
                <td class="half">@lang('invoice.totalWithoutTax'): <span style="float:right;line-height: 1;font-size: 16px;">{{number_format(round($invoice->totalPrice, 2), 2, $decimalFormat, $thousandsFormat)}} {{$invoice->getCurrencySymbol()['symbol']}}</span></td>
            </tr>
            @php($tmpTaxDiff = 0)
            @if(!empty($invoice->tax_data))
                @foreach($invoice->tax_data as $taxRate => $taxValue)
                    ...
                    <tr>
                        <td class="half"></td>
                        <td class="half">@lang('invoice.tax') {{$taxRate}}%: <span style="float:right;line-height: 1;font-size: 16px;">{{number_format(round($taxValue, 2), 2, $decimalFormat, $thousandsFormat)}} {{$invoice->getCurrencySymbol()['symbol']}}</span></td>
                    </tr>
                @endforeach
            @endif
            @if(!empty(round($tmpTaxDiff, 2)))
                <tr>
                    <td class="half"></td>
                    <td class="half">@lang('invoice.roundingDifference'): <span style="float:right;line-height: 1;font-size: 16px;">{{number_format(round($tmpTaxDiff, 2), 2, $decimalFormat, $thousandsFormat)}} {{$invoice->getCurrencySymbol()['symbol']}}</span></td>
                </tr>
            @endif
        @endif
        @if($invoice->payment === 'CASH' && $invoice->currency_3_code === 'EUR')
            <tr>
                <td class="half"></td>
                <td class="half">@lang('invoice.cash_payment_rounding'): <span style="float:right;line-height: 1;font-size: 16px;">{{number_format(round($invoice->cash_payment_rounding,2), 2, $decimalFormat, $thousandsFormat)}} {{$invoice->getCurrencySymbol()['symbol']}}</span></td>
            </tr>
        @endif
        @if($showTax || $invoice->payment === 'CASH' && $invoice->currency_3_code === 'EUR')
            <tr style="">
                <td class="half"></td>
        ...
        <tr>
            ...
            <td class="half sum_total">
                @lang('invoice.sum_total'): <span style="float:right;">{{number_format(round($showTax ? $invoice->getTotalPriceWithTaxAndCashPaymentRounding() : $invoice->getTotalPriceWithCashPaymentRounding(),2), 2, $decimalFormat, $thousandsFormat)}} {{$invoice->getCurrencySymbol()['symbol']}}</span></td>
        </tr>
        <tr>
            <td class="half"></td>
